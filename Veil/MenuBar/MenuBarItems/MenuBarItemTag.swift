...
struct MenuBarItemTag: Hashable, CustomStringConvertible {
    ...
    var isSystemItem: Bool {
        switch namespace {
        case .controlCenter, .systemUIServer, .textInputMenuAgent, .weather, .passwords, .screenCaptureUI, .ssMenuAgent, .veil, .gamePolicyAgent:
            return true
    ...
    private init(controlItem identifier: ControlItem.Identifier) {
        self.init(namespace: .veil, title: identifier.rawValue, instanceIndex: 0)
    }
}
...
extension MenuBarItemTag {
    ...
    /// An array of tags for items representing Veil's control items.
    static let controlItems = ControlItem.Identifier.allCases.map(\.tag)

    // MARK: Control Items

    /// The tag for Veil's control item for the "Visible" section.
    static let visibleControlItem = MenuBarItemTag(controlItem: .visible)

    /// The tag for Veil's control item for the "Hidden" section.
    static let hiddenControlItem = MenuBarItemTag(controlItem: .hidden)

    /// The tag for Veil's control item for the "Always-Hidden" section.
...
extension MenuBarItemTag.Namespace {
    /// The namespace for the "Veil" process.
    static let veil = string(Constants.bundleIdentifier)
