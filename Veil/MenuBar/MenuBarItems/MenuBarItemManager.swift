...
final class MenuBarItemManager: ObservableObject {
    ...
    private nonisolated(unsafe) var menuOpenCheckTask: Task<Bool, Never>?
...
extension MenuBarItemManager {
    ...
    /// Temporarily moves `item` into the visible area next to the Veil icon,
    ...
    private func relocateNewLeftmostItems(
        ...
        let leftmostItems = items
            .filter {
                ...
                // Include normal items AND the Veil icon (which is a control item).
        ...
        // The Veil icon must always appear in the visible section.
        if let veilIcon = leftmostItems.first(where: { $0.tag == .visibleControlItem }) {
            MenuBarItemManager.diagLog.info("Relocating Veil icon \(veilIcon.logString) to visible section")
            do {
                try await move(
                    item: veilIcon,
                ...
                MenuBarItemManager.diagLog.error("Failed to relocate Veil icon \(veilIcon.logString): \(error)")
                return false
            }
            return true
        }
        ...
        // resolved sourcePIDs will handle relocation safely. The Veil-icon
...
extension MenuBarItemManager {
    ...
    func applyProfileLayout(
        ...
        // placed in the visible section next to the Veil visible control
        ...
        if !unmanagedUIDs.isEmpty {
            // Insert screen-right of the Veil visible control icon.
