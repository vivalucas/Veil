...
final class MenuBarSection {
    ...
    /// A Boolean value that indicates whether the Veil Bar should be used
    ...
    /// The best screen to show the Veil Bar on.
    ///
    ...
    func show(triggeredByHotkey: Bool = false) {
        ...
        // Determine whether we should use the Veil Bar based on settings.
        ...
        if shouldUseIceBarBasedOnSettings {
            ...
            case .iceBar:
                diagLog.info("Not enough space to show items inline, falling back to Veil Bar")
            }
        } else {
            preferredPresentationMode = .inline
        }

        // Use Ice Veil if settings say so OR if items still won't fit inline.
        if preferredPresentationMode == .iceBar {
            // Make sure hidden and always-hidden control items are collapsed.
            // Still update the visible control item (Veil icon) state to show
            // its alternate icon.
        ...
        // If we made it here, we're not using the Veil Bar.
    ...
    func hide() {
        ...
        menuBarManager.iceBarPanel.close() // Make sure Veil Bar is always closed.
