...
extension HIDEventManager {
    ...
    private func handleShowOnClick(appState: AppState, screen: NSScreen, clickLocation: CGPoint, modifierFlags: NSEvent.ModifierFlags, isDoubleClick: Bool = false) {
        ...
        // on a widget's icon don't also trigger Veil's show-on-click reveal.
        ...
        if isDoubleClick {
            ...
            {
                // If the always-hidden section is currently showing via the Veil
                // Bar, a plain click should close it rather than switch the Veil
    ...
    private func handleSmartRehide(
        ...
        // Make sure clicking the Veil icon doesn't trigger rehide.
        ...
        // Only continue if the click is not inside the Veil Bar, at
    ...
    /// using the system-wide accessibility hit-test. Excludes Veil's own
    ...
    /// role). When the helper returns true, Veil defers to the widget under
    ...
    ///
    /// After a profile change with VeilBar active, the Window Server tracks
...
extension HIDEventManager {
    ...
    /// A Boolean value that indicates whether the mouse pointer is within
    /// the bounds of the Veil Bar panel.
