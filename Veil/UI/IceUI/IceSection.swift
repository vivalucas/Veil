...
struct IceSection<Header: View, Content: View, Footer: View>: View {
    ...
    private var glassFill: Color {
        Color(red: 0.9, green: 0.97, blue: 1.0).opacity(0.16)
    }
        ...
        Section {
            VStack(alignment: .leading, spacing: 0) {
                ...
                if isBordered {
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 20, style: .continuous)
                            .strokeBorder(.white.opacity(0.48), lineWidth: 0.6)
                    )
                    .overlay(
                        RoundedRectangle(cornerRadius: 20, style: .continuous)
                            .strokeBorder(glassStroke, lineWidth: 1.0)
                    )
                    .shadow(color: Color(red: 0.08, green: 0.32, blue: 0.68).opacity(0.07), radius: 10, y: 4)
                } else {
                    contentLayout
                }
...
private struct IceSectionDivider: View {
    var body: some View {
        Divider()
            .overlay(Color(red: 0.6, green: 0.82, blue: 1.0).opacity(0.22))
