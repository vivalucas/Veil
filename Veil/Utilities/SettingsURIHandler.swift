...
/// Handles settings manipulation via veil:// URLs with whitelist-based security.
@MainActor
enum SettingsURIHandler {
    ...
    /// Handles veil://set?key=X&value=Y&type=bool URL.
    ...
    /// Schemes that can safely receive settings callbacks locally.
    ///
    /// Settings payloads can include the complete app configuration, so network
    /// URL schemes are intentionally excluded to avoid accidental exfiltration.
    static func isAllowedCallbackScheme(_ scheme: String) -> Bool {
        let normalized = scheme.lowercased()
        guard !normalized.isEmpty else { return false }
        guard normalized != "veil" else { return false }
        guard normalized != "http", normalized != "https" else { return false }
        guard !normalized.hasPrefix("x-apple-"), !normalized.hasPrefix("x-apple.") else {
            return false
        }
    ...
    private static func sendCallbackResponse(response: [String: Any], callback: String) -> Bool {
        ...
        // Only allow local custom app schemes. Do not send settings data to
        // web or system schemes.
        if !isAllowedCallbackScheme(scheme) {
...
extension Notification.Name {
    ...
    static let settingsDidChangeViaURI = Notification.Name("io.github.vivalucas.Veil.settingsDidChangeViaURI")

    /// Posted when a per-display setting is changed externally via Settings URI scheme.
    static let perDisplaySettingsDidChangeViaURI = Notification.Name("io.github.vivalucas.Veil.perDisplaySettingsDidChangeViaURI")

    /// Posted when a get request response is broadcast via distributed notification.
    static let settingsURIGetResponse = Notification.Name("io.github.vivalucas.Veil.settingsURIGetResponse")

    /// Posted when the Settings URI whitelist changes.
    static let settingsURIWhitelistDidChange = Notification.Name("io.github.vivalucas.Veil.settingsURIWhitelistDidChange")
}
