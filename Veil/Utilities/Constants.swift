...
/// App-specific constants for the main app target.
...
enum Constants {
    ...
    static let repositoryURLInfoPlistKey = "VeilRepositoryURL"

    /// Info.plist key used to configure the executable URI for
    /// `MenuBarItemSpacingManager` shell commands.
    static let menuBarItemSpacingExecutableURIInfoPlistKey = "VeilMenuBarItemSpacingExecutableURI"
    ...
    /// The URL for the latest GitHub release.
    ...
    /// Extracted so localized strings can use a placeholder instead of a unicode arrow.
