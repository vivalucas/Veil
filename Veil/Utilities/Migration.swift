...
// MARK: - Migrate Per-Display Veil Bar

extension MigrationManager {
    /// Migrates legacy global Veil Bar settings to per-display configurations.
    private func migratePerDisplayIceBar() -> MigrationResult {
        ...
        // Only create per-display configs if the user had Veil Bar enabled.
        guard useIceBar else {
            Defaults.set(true, forKey: .hasMigratedPerDisplayIceBar)
            diagLog.info("Per-display Veil Bar migration: Veil Bar was disabled, nothing to migrate")
            return .success
        }
        ...
        do {
            ...
            Defaults.set(true, forKey: .hasMigratedPerDisplayIceBar)
            diagLog.info("Per-display Veil Bar migration: migrated \(configs.count) display(s)")
        } catch {
    ...
    enum MigrationError: Error, CustomStringConvertible {
        ...
        var description: String {
            ...
            case let .perDisplayIceBarMigrationError(error):
                "Error migrating per-display Veil Bar configuration: \(error)"
