...
///
/// Log files are written to `~/Library/Logs/Veil/`.
final class DiagnosticLogger: @unchecked Sendable {
    ...
    private let osLog = Logger(
        subsystem: Bundle.main.bundleIdentifier ?? "io.github.vivalucas.Veil",
    ...
    private let writeQueue = DispatchQueue(
        label: "io.github.vivalucas.Veil.DiagnosticLogger.writeQueue",
        qos: .utility
    )
    ...
    private func openLogFile() {
        ...
        let fileName = "veil_\(fileNameFormatter.string(from: Date())).log"
